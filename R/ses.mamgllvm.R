#' Standardized effect size of relative importance values for mamgllvm
#'
...
#'
#' @export
#' @param data Data frame, typically of environmental variables. Rows for sites and colmuns for environmental jariables.
...
#' ses.mamgllvm(data = env_assem, y = "pre.abs",
...
ses.mamgllvm <- function(data, y, family, scale = TRUE,
                      rank = NULL, par = FALSE, runs = 999){
  if (scale) data <- as.data.frame(scale(data))
  res.obs <- mamgllvm(data,y = y,scale = scale, family,
