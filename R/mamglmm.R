...
for (i in 1:n.vars){
	...
	f.str <- make.formula2(y, vars.temp, rand_intercept)
		# if (family=="gaussian") fit.temp <- manylm(f.str,data=data2)
	ncol_y <- ncol(get(y))
	log.L.temp <- numeric(ncol_y)
			for (l in 1:ncol_y){
				suppressWarnings(log.L.temp[l] <- my.glmm(y,l,f.str=f.str,data=data2,family=family))
				}
# return(log.L.temp)
