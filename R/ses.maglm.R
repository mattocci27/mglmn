ses.maglm <- function(data,y,family,scale=TRUE,AIC.restricted=TRUE,par=FALSE, runs=999){
...
# AIC.restricted=F
  if (par==FALSE) {
    res.rand0 <- sapply(null.env.list,function(x){mamglm(x,y=y,family,AIC.restricted=AIC.restricted)$importance})}
